...
MERGE INTO DIM_SALES_REP AS tgt
USING (
  SELECT DISTINCT
    UPPER(TRIM(SALES_REP)) AS SALES_REP_NAME
  FROM STG.RAW_SALES
  WHERE SALES_REP IS NOT NULL AND TRIM(SALES_REP) <> ''
) AS src
ON tgt.SALES_REP_NAME = src.SALES_REP_NAME
WHEN NOT MATCHED THEN
  INSERT (SALES_REP_NAME) VALUES (src.SALES_REP_NAME);